...
export_method_panel <- function(analysis, base_name, width = 12, height = 10) {
  ...
  grDevices::pdf(file.path(figure_dir, paste0(base_name, "_method_maps.pdf")), width = width, height = height)
  graphics::par(mfrow = c(2, 2), mar = c(4.2, 4.2, 2.6, 1.2))
...
utils::write.csv(clinical_analysis$comparison, file.path(output_dir, "clinical_method_comparison.csv"), row.names = FALSE)
utils::write.csv(yeast_analysis$comparison, file.path(output_dir, "yeast_method_comparison.csv"), row.names = FALSE)

utils::write.csv(clinical_analysis$summary$variable_summary, file.path(output_dir, "clinical_variable_summary.csv"), row.names = FALSE)
utils::write.csv(yeast_analysis$summary$variable_summary, file.path(output_dir, "yeast_variable_summary.csv"), row.names = FALSE)

utils::write.csv(clinical_analysis$summary$label_frequency, file.path(output_dir, "clinical_label_frequency.csv"), row.names = FALSE)
utils::write.csv(yeast_analysis$summary$label_frequency, file.path(output_dir, "yeast_label_frequency.csv"), row.names = FALSE)

utils::write.csv(clinical_analysis$summary$richest_objects, file.path(output_dir, "clinical_richest_objects.csv"), row.names = FALSE)
utils::write.csv(yeast_analysis$summary$richest_objects, file.path(output_dir, "yeast_richest_objects.csv"), row.names = FALSE)
