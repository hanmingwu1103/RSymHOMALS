...
write.csv(benchmark$raw, file.path(output_dir, "scalability_benchmark_raw.csv"), row.names = FALSE)
write.csv(benchmark$summary, file.path(output_dir, "scalability_benchmark_summary.csv"), row.names = FALSE)
...
plot_scalability <- function(summary_df) {
  ...
  grDevices::pdf(file.path(latex_fig_dir, "scalability_benchmarks.pdf"), width = 10.5, height = 4.8)
